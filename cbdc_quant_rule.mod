...
parameters
	...
	m_dc_to_gdp (long_name='CBDC to GDP ratio')
...
load calibrated_parameters_ssm.mat
xi_F = xi_F_c;
xi_I = xi_I_c;
zzeta = zzeta_c;

gamma_g = 1;
% m_dc_to_gdp = 0.338001460713950;
m_dc_to_gdp = x_m_dc_to_gdp;
...
model;
	...
	*((exp(Inv_t(+1))/exp(Inv_t))-1);
	...
	[name='17. Regular money FOC']
	(Ac_t*exp(vc_t)^2+AInv_t*exp(vInv_t)^2-2*B)*theta*(exp(m_c_t))^(theta-1) = 
	(i_t)/(1+i_t);

	[name='18. Digital money FOC']
	T^(theta) = ((i_t - i_dc_t)/i_t)*((exp(m_dc_t))/exp(m_c_t))^(1-theta);
	...
	* exp(e_i_t*sig_i);
	...
	[name='30. Second monetary policy']
	m_dc_t = log(m_dc_to_gdp*exp(gdp_t)) + log((1+pi_obj)/(1+pi_t))*(phi_pi_2);
	...
	[name='41. Real GDP']
	exp(gdp_t) = (exp(c_t)+exp(Inv_t))/(1-g_t);
	...
	log(a_t) = (rho_a)*log(a_t(-1)) + (1-rho_a)*log(1) + e_a_t*sig_a;
	...
	log(S_TC_t) = rho_S_TC*log(S_TC_t(-1)) + e_S_TC_t*sig_S_TC;
	...
	g_t = rho_g*g_t(-1) + (1-rho_g)*g_ss + e_g_t*sig_g;
	...
	-(tau_ls_ss+tau_c_ss*exp(c_ss)+tau_r_ss*r_ss*exp(k_ss)+tau_w_ss*exp(w_F_ss)
		*exp(L_F_ss))=
	gamma_g*((1+pi_t)^(-1)*((1+i_t)*d_t+exp(m_c_t)+(1+i_dc_t)
		*(exp(m_dc_t)))-((1+pi_obj)^(-1)*((1+i_ss)*d_ss+exp(m_c_ss)
			+(1+i_dc_ss)*(m_dc_ss))));

	[name='69. Aggregate vacancies']
	exp(vac_t) = exp(vac_F_t) + exp(vac_I_t);

	[name='70. Aggregate matchings']
	exp(match_t) = (exp(un_t))^(omega)*(exp(vac_F_t))^(1-omega) 
	+ (exp(un_t))^(omega)*(exp(vac_I_t))^(1-omega);

	[name='71. Total factor productivity']
	exp(TFP_t) = exp(Y_t)/((exp(k_t(-1)))^(alpha_k)*(exp(L_t))^(1-alpha_k));

	[name='72. Average transaction costs distortion']
	exp(tc_avg_t) = ((1+2*(Ac_t*exp(vc_t)-sqrt(A*B)))*exp(c_t)
		+(1+2*(AInv_t*exp(vInv_t)-sqrt(A*B)))*exp(Inv_t))
	/(exp(c_t)+exp(Inv_t));

	[name='73. Average real wage of the economy']
...
cbdc_quant_rule_full_ss = oo_.dr.ys;
save cbdc_quant_rule_full_ss.mat cbdc_quant_rule_full_ss;
