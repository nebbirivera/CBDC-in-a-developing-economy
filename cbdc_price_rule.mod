...
var
	...
	vac_t (long_name='Aggregate vacancies')
	...
	m_dc_t (long_name='Central Bank Digital Currency')
	...
	i_dc_t (long_name='2nd monetary policy nominal net interest rate')
	...
	LGF_m_dc_t (long_name='LGF derivative w.r.t. m_dc_t')
...
parameters
	...
	rho_a_F (long_name='Persistance of formal product. shock')
	sig_a (long_name='TFP shock standard deviation')
	sig_S_TC (long_name='Demand for liquidity shock standard deviation')
	...
	m_dc_ss (long_name='Levels steady state CBDC')
	i_dc_ss (long_name='Levels steady state CBDC interest rate')
	...
	i_spread (long_name='GDP steady state')
;

varexo e_i_t e_i_dc_t e_S_TC_t e_a_t e_a_F_t e_z_u_t e_g_t;
...
model;
	...
	*((exp(Inv_t(+1))/exp(Inv_t))-1);
	...
	exp(V_F_f_t) = exp(p_F_t)*a_F_t 
	- exp(w_F_t)*(1+tau_w_t) + (1-eta_F)*exp(SDF_t)*exp(V_F_f_t(+1));
	...
	zzeta*z_u_t*exp(un_t) = 
	exp(pr_F_t)*exp(lambda_F_t) + exp(pr_I_t)*exp(lambda_I_t);

	[name='17. m_c_t FOC']
	(Ac_t*exp(vc_t)^2 + AInv_t*exp(vInv_t)^2 - 2*B)*exp(LGF_m_c_t) = 
	(i_t)/(1+i_t);

	[name='18. exp(m_dc_t) FOC']
	(exp(LGF_m_dc_t)/exp(LGF_m_c_t))*i_t = i_t - i_dc_t;
	...
	[name='24. exp(aux_p_1_t)']
	exp(aux_p_1_t) = exp(mc_t)*exp(Y_t) 
	+ phi_p*exp(SDF_t)*(1+pi_t(+1))^(eps_p)*exp(aux_p_1_t(+1));

	[name='25. exp(aux_p_2_t)']
	exp(aux_p_2_t) = exp(Y_t) 
	+ phi_p*exp(SDF_t)*(1+pi_t(+1))^(eps_p-1)*exp(aux_p_2_t(+1));

	[name='26. Production function']
	exp(Y_t) = 
	(a_t/exp(ups_p_t))*(exp(k_t(-1)))^(alpha_k)*(exp(y_L_t))^(1-alpha_k);
	...
	* exp(sig_i*e_i_t);
	...
	[name='30. Second monetary policy Taylor rule']
	i_dc_t = ((i_ss)/(i_ss/(i_ss-i_spread)))
	*((pi_t)/(pi_obj))^(-phi_pi_2) * exp(e_i_dc_t);
	...
	[name='40. LGF']
	exp(LGF_t) = (exp(m_c_t))^(theta) + (T*exp(m_dc_t))^(theta);

	[name='41. LGF derivative w.r.t. exp(m_c_t)']
	exp(LGF_m_c_t) = theta*(exp(m_c_t))^(theta-1);

	[name='42. LGF derivative w.r.t. exp(m_dc_t)']
	exp(LGF_m_dc_t) = theta*(exp(m_dc_t))^(theta-1)*T^(theta);

	[name='43. Prob. a vacancy matches with a worker in the formal sector']
	exp(q_F_t) = (exp(un_t)/exp(vac_F_t))^(omega);

	[name='44. Prob. a vacancy matches with a worker in the informal sector']
	exp(q_I_t) = (exp(un_t)/exp(vac_I_t))^(omega);

	[name='45. Prob. a worker find a vacancy in the formal sector']
	exp(pr_F_t) = (exp(un_t)/exp(vac_F_t))^(omega-1);

	[name='46. Prob. a worker find a vacancy in the informal sector']
	...
	log(a_F_t) = rho_a_F*log(a_F_t(-1)) + sig_a_F*e_a_F_t;

	[name='49. AR(1) General TC demand shock']
	log(S_TC_t) = rho_S_TC*log(S_TC_t(-1)) + sig_S_TC*e_S_TC_t;
	...
	log(z_u_t) = rho_u*log(z_u_t(-1)) + sig_u*e_z_u_t;

	[name='60. AR(1) Formal sector payroll taxes']
	tau_w_t = rho_a*tau_w_t(-1) + (1-rho_a)*tau_w_ss;

	[name='61. AR(1) VAT']
	tau_c_t = rho_a*tau_c_t(-1) + (1-rho_a)*tau_c_ss;

	[name='62. AR(1) Capital return rate']
	tau_r_t = rho_a*tau_r_t(-1) + (1-rho_a)*tau_r_ss;
	...
	[name='68. Government expenditure']
	exp(G_t) = g_t*exp(gdp_t);

	[name='69. Government budget constraint']
	tau_ls_t + tau_c_t*exp(c_t) + tau_r_t*r_t*exp(k_t(-1)) 
	+ tau_w_t*exp(w_F_t)*exp(L_F_t) + exp(m_c_t) + exp(m_dc_t) + d_t = 
	(1+pi_t)^(-1)
	*((1+i_t(-1))*d_t(-1) + exp(m_c_t(-1)) + (1+i_dc_t(-1))*exp(m_dc_t(-1))) 
	+ exp(G_t);
	...
	gamma_g*((1+pi_t)^(-1)*((1+i_t(-1))*d_t(-1)
		+exp(m_c_t(-1))+(1+i_dc_t(-1))*exp(m_dc_t(-1))) 
	- ((1+pi_obj)^(-1)*((1+i_ss)*d_ss+exp(m_c_ss)+(1+i_dc_ss)*exp(m_dc_ss))));

	[name='71. Aggregate vacancies']
	exp(vac_t) = exp(vac_F_t) + exp(vac_I_t);

	[name='72. Aggregate vacancies']
	exp(match_t) = (exp(un_t))^(omega)*(exp(vac_F_t))^(1-omega) 
	+ (exp(un_t))^(omega)*(exp(vac_I_t))^(1-omega);

	[name='73. Total factor productivity']
	exp(TFP_t) = exp(Y_t)/((exp(k_t(-1)))^(alpha_k)*(exp(L_t))^(1-alpha_k));

	[name='74. Average transaction costs distortion']
	exp(tc_avg_t) = ((1+2*(Ac_t*exp(vc_t)-sqrt(A*B)))*exp(c_t)
		+(1+2*(AInv_t*exp(vInv_t)-sqrt(A*B)))*exp(Inv_t))
	/(exp(c_t)+exp(Inv_t));

	[name='75. Average real wage of the economy']
	exp(w_avg_t) = exp(w_F_t)*(1-infor_rate_t)+exp(w_I_t)*infor_rate_t;
end;

shocks;
	var e_i_t; stderr 1; 
	var e_i_dc_t; stderr 0; 
...
cbdc_price_rule_full_ss = oo_.dr.ys;
save cbdc_price_rule_full_ss.mat cbdc_price_rule_full_ss;
