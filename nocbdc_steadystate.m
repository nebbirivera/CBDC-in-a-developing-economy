...
function [ys,params,check] = nocbdc_steadystate(ys,exo,M_,options_)
	...
	aux_mc = (((1+pi_t)*(1-phi_p*(1+pi_t)^(eps_p-1)*betta))/...
		(1-phi_p*(1+pi_t)^(eps_p)*betta))^(-1);
	...
	lab_force_part_rate = 0.655850371642851;
	inf_rate = 0.430278065206939; 
	un_rate = 0.112494100209431; 
	...
	y_I_t = a_I*L_I_t;
	y_F_t = a_F_t*L_F_t;
	y_L_t = (y_F_t^((eps_L-1)/eps_L) + y_I_t^((eps_L-1)/eps_L))^...
		(eps_L/(eps_L-1));
	...
	fixed(14) = un_t;
	fixed(15) = g_t;
	% 16th position reserved for xi_F
	fixed(17) = eta_F;
	% 18th position reserved for xi_I
	fixed(19) = eta_I;
	fixed(20) = omega;
	fixed(21) = T;
	fixed(22) = L_F_t;
	...
	% Get initial guess values from lin. appx. solution for numeric solver
	[c_0 k_0 m_c_0] = nocbdc_initial_guess(fixed, 2*ones(3,1));
	x0 = [c_0 k_0 m_c_0];

	% Set numeric solver options
	options = optimset('Display', 'off', 'TolX', 1e-10, 'TolFun', 1e-10,...
		'MaxFunEvals',1e4,'MaxIter',1e4);
	% Run numeric solver in order to solve reduced non-linear system
	[x1 Fval] = fsolve(@(x) nocbdc_num_eqs(x, fixed), x0, options);

	% Assign results from numeric solver
	c_t = x1(1);
	k_t = x1(2);
	m_c_t = x1(3);
	% Assing the rest of the variables with closed form solutions
	Inv_t = delta*k_t;
	...
	lambda_c_t = (u_c_t + gama*lambda_X_t)/((1+tau_c_t)*...
		(1+2*(Ac_t*vc_t-sqrt(A*B))));
	p_L_t = (1-alpha_k)*mc_t*Y_t/y_L_t;
	p_F_t = p_L_t*(y_L_t/y_F_t)^(1/eps_L);
	p_I_t = p_L_t*(y_L_t/y_I_t)^(1/eps_L);
	V_I_f_t = (muu/(1-(1-eta_I)*betta))*(p_I_t*a_I_t -...
		pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t);
	w_I_t = p_I_t*a_I_t - V_I_f_t*(1-(1-eta_I)*betta);
	V_I_h_t = (w_I_t - pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t)/...
		(1-(1-eta_I)*betta);
	lambda_I_t = V_I_h_t*lambda_c_t;
	V_F_f_t = muu*(p_F_t*a_F_t - (1+tau_w_t)*pssi*c_t*...
		L_t^(chi)*u_c_t/lambda_c_t)/((1+tau_w_t*(1-muu))*(1-(1-eta_F)*betta));
	...
	d_ss = 0;
	d_t = d_ss;
	tau_ls_ss = (1+pi_t)^(-1)*((i_t-pi_t)*d_t+(-pi_t)*m_c_t) + G_t -...
		(tau_c_t*c_t + tau_r_t*r_t*k_t + tau_w_t*w_F_t*L_F_t);
	tau_ls_t = tau_ls_ss;
	vac_t = vac_F_t + vac_I_t;
	match_t = (un_t)^(omega)*(vac_F_t)^(1-omega)+(un_t)^(omega)*...
		(vac_I_t)^(1-omega);
	TFP_t = Y_t/((k_t)^(alpha_k)*(L_t)^(1-alpha_k));
	tc_avg_t = ((1+2*(Ac_t*vc_t-sqrt(A*B)))*c_t+(1+...
		2*(AInv_t*vInv_t-sqrt(A*B)))*Inv_t)/(c_t+Inv_t);
