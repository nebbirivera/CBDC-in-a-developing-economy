...
function [ys,params,check] = cbdc_quant_rule_steadystate(ys,exo,M_,options_)
...
aux_mc = (((1+pi_t)*(1-phi_p*(1+pi_t)^(eps_p-1)*betta))/(1-phi_p*(1+pi_t)^(eps_p)*betta))^(-1);%6
mc_t = ((eps_p-1)/eps_p)*(1+pi_opt_t)*aux_mc;
ups_p_t = ((1-phi_p)*(1+pi_opt_t)^(-eps_p)*(1+pi_t)^(eps_p))/(1-phi_p*(1+pi_t)^(eps_p));
SDF_t = betta;
a_I_t = a_I;
Ac_t = A;
AInv_t = A;
...
% Labor part steady state targets for keeping calibrated parameters
load cbdc_qr_labor_rates_ss.mat
lab_force_part_rate = cbdc_qr_lfpr_ss;
inf_rate = cbdc_qr_infor_rate_ss;
un_rate = cbdc_qr_unemp_rate_ss;
...
% Start parameters for refining calibration
flag_accuracy = false;
last_step_size = 1;
xi_I_prev = xi_I;
xi_F_prev = xi_F;
...
while flag_accuracy ~= true
	...
	[c_0 k_0 i_dc_0] = cbdc_quant_rule_initial_guess(fixed, ones(3,1), m_dc_to_gdp);
	x0 = [double(c_0) double(k_0) double(i_dc_0)];
	[x1 Fval] = fsolve(@(x) cbdc_quant_rule_num_eqs(x, fixed, m_dc_to_gdp), x0, options);
	c_t = x1(1);
	k_t = x1(2);
	i_dc_ss = x1(3);
	i_dc_t = i_dc_ss;
	...
	if all(~isreal(x1))
		display("ERROR*: Numeric solution has complex values.")
		check = 1;
		x1
		return;
	end
	...
	lambda_c_t = (u_c_t + gama*lambda_X_t)/((1+tau_c_t)*(1+2*(Ac_t*vc_t-sqrt(A*B))));
	p_L_t = (1-alpha_k)*mc_t*Y_t/y_L_t;
	p_F_t = p_L_t*(y_L_t/y_F_t)^(1/eps_L);
	p_I_t = p_L_t*(y_L_t/y_I_t)^(1/eps_L);
	V_I_f_t = (muu/(1-(1-eta_I)*betta))*(p_I_t*a_I_t - pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t);
	w_I_t = p_I_t*a_I_t - V_I_f_t*(1-(1-eta_I)*betta);
	V_I_h_t = (w_I_t - pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t)/(1-(1-eta_I)*betta);
	lambda_I_t = V_I_h_t*lambda_c_t;
	V_F_f_t = muu*(p_F_t*a_F_t - (1+tau_w_t)*pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t)/((1+tau_w_t*(1-muu))*(1-(1-eta_F)*betta));
	w_F_t = (p_F_t*a_F_t - (1-(1-eta_F)*betta)*V_F_f_t)/(1+tau_w_t);
	V_F_h_t = (w_F_t - pssi*c_t*L_t^(chi)*u_c_t/lambda_c_t)/(1-(1-eta_F)*betta);

	% Calibration of xi_i by reassigning their values
	xi_I = V_I_f_t*q_I_t;
	xi_F = V_F_f_t*q_F_t;
	...
	lambda_I_t = V_I_h_t*lambda_c_t;
	% Calibration of zzeta by reassigning its value
	zzeta = (pr_F_t*lambda_F_t + pr_I_t*lambda_I_t)/un_t;

	last_step_size = norm([xi_I - xi_I_prev, xi_F - xi_F_prev, zzeta - zzeta_prev]);
...
% r_t = alpha_k*mc_t*Y_t/k_t;
r_t = (1+2*(A*(Inv_t/(LGF_t))-sqrt(A*B)))*((1/betta)-1+delta)/(1-tau_r_t);
gdp_t = (c_t+Inv_t)/(1-g_t);
G_t = g_t*gdp_t;
d_ss = 0;
d_t = d_ss;
tau_ls_ss = (1+pi_t)^(-1)*((i_t-pi_t)*d_t+(-pi_t)*m_c_t) + G_t - (tau_c_t*c_t + tau_r_t*r_t*k_t + tau_w_t*w_F_t*L_F_t);
tau_ls_t = tau_ls_ss;
vac_t = vac_F_t + vac_I_t;
match_t = (un_t)^(omega)*(vac_F_t)^(1-omega)+(un_t)^(omega)*(vac_I_t)^(1-omega);
TFP_t = Y_t/((k_t)^(alpha_k)*(L_t)^(1-alpha_k));
tc_avg_t = ((1+2*(Ac_t*vc_t-sqrt(A*B)))*c_t+(1+2*(AInv_t*vInv_t-sqrt(A*B)))*Inv_t)/(c_t+Inv_t);
